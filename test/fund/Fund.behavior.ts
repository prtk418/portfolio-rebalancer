...
export function shouldBehaveLikeFund(): void {
  it("should return the new greeting once it's changed", async function () {
    ...
    const usdcContract = new ethers.Contract("0xd35CCeEAD182dcee0F148EbaC9447DA2c4D449c4", ERC20ABI.abi, usdcSigner);
    await usdcContract.transfer(this.signers.admin.address, ethers.utils.parseUnits("10000000000000", 6));
    expect(await usdcContract.balanceOf(this.signers.admin.address)).to.equal(
    ...
    await usdcContract
      ...
      .approve(uniswapRouterContract.address, ethers.utils.parseUnits("10000000000", 6));
    await this.quoteToken1
      .connect(this.signers.admin)
      .approve(uniswapRouterContract.address, ethers.utils.parseUnits("1000000"));
    ...
    expect(ethers.utils.formatUnits(await this.quoteToken1.balanceOf(this.fund.address))).to.equal(
      "332.222924578076323389",
    );
    expect(ethers.utils.formatUnits(await this.quoteToken2.balanceOf(this.fund.address))).to.equal(
      "66.462249141842416382",
    );
    expect(ethers.utils.formatUnits(await this.quoteToken3.balanceOf(this.fund.address))).to.equal(
      "33.232228915260060568",
    );
    ...
    await this.fund.rebalance();
    expect(ethers.utils.formatUnits(await this.quoteToken1.balanceOf(this.fund.address))).to.equal(
      "664.224473364450538984",
    );
    expect(ethers.utils.formatUnits(await this.quoteToken2.balanceOf(this.fund.address))).to.equal(
      "132.915681885983983478",
    );
    expect(ethers.utils.formatUnits(await this.quoteToken3.balanceOf(this.fund.address))).to.equal(
      "66.462267798990419342",
    );
    ...
    await this.fund.rebalance();
    expect(ethers.utils.formatUnits(await this.quoteToken1.balanceOf(this.fund.address))).to.equal(
      "442.426581886659445905",
    );
    expect(ethers.utils.formatUnits(await this.quoteToken2.balanceOf(this.fund.address))).to.equal(
      "177.158287374022696288",
    );
    expect(ethers.utils.formatUnits(await this.quoteToken3.balanceOf(this.fund.address))).to.equal(
      "88.587021508718240915",
    );
    ...
    await this.fund.rebalance();
    expect(ethers.utils.formatUnits(await this.quoteToken1.balanceOf(this.fund.address))).to.equal(
      "368.686858397682913184",
    );
    expect(ethers.utils.formatUnits(await this.quoteToken2.balanceOf(this.fund.address))).to.equal(
      "293.780645388497877452",
    );
    expect(ethers.utils.formatUnits(await this.quoteToken3.balanceOf(this.fund.address))).to.equal(
      "73.926557727516686298",
    );
    expect(ethers.utils.formatUnits(await usdcContract.balanceOf(this.fund.address), 6)).to.equal("0.0");
