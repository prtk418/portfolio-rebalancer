...
export async function deployFundFixture(): Promise<{ fund: Fund }> {
  ...
  let quoteToken1: MyToken = <MyToken>await quoteTokenFactory1.connect(admin).deploy();
  ...
  let quoteToken2: MyToken = <MyToken>await quoteTokenFactory2.connect(admin).deploy();
  ...
  let quoteToken3: MyToken = <MyToken>await quoteTokenFactory3.connect(admin).deploy();
